...
SMODS.Joker{ -- Black Diamond
    key = 'black', --joker key
    loc_txt = {
        name = '{C:default,E:1,s:1.5}Black Diamond{}',
        text = {
            '{C:attention}Immediately{} defeat selected {C:attention}blind{}.', 
            '{C:inactive}pretty buggy atm (WIP){}',
            '{C:legendary,s:0.7,E:1}- From \"You don\'t wanna know...\"{}'
        }
    },
    ...
    calculate = function(self, card, context)
        ...
        -- Swallow the built-in end-of-round
        -- (by returning true, we stop other blind logic from running now)
        -- Then roll our own 3-sec delayed win:
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            delay   = 0,   -- wait 3 seconds
            func    = function()

            -- Force the blind to score for us:
                G.GAME.chips        = G.GAME.blind.chips
                G.STATE             = G.STATES.HAND_PLAYED
                G.STATE_COMPLETE    = true
                end_round()        -- finish the round
            return true
            end
        }))
        return true
